...
# https://www.youtube.com/watch?v=tJw8JEX89eo
...
# Meter Type 2 - A circular style meter with a scale and a curved 101 segment bar.
class meterType2:
    def __init__(self, mt2Pos = vector(0, 0, 0), mt2Color = color.blue, mt2ScaleMin = 0, mt2ScaleMax = 100, mt2Label = "", mt2Units = ""):
        ...
        self.meterSegments = [] # A list in which to put all the meter segments for later reference and update.
...
class meterType3:
    ...
    def update(self, mt3Value = "nan"):
        ...
        if mt3Value != "nan":
            ...
            # Update the meter reading - basically converting the measurement to a proportion of the unit length column.
            self.measurement.axis = vector(0, 0.15 + ((self.mt3Value - self.mt3ScaleMin)  / self.mt3Range), 0)
        else:
            # Turn on the data warning.
            self.DataWarning.opacity = 1

# Meter Type 4 - A 10 segment LED bank style meter with blue, green and red LEDs that illuminate from left to right, or bottom to top..
class meterType4:
    def __init__(self, mt4Pos = vector(0, 0, 0), mt4InARow = True, mt4OffColor = color.gray(0.5), mt4ScaleMin = 0.0, mt4ScaleMax = 100.0):
        ...
        # Draw the LED bank...
        if self.mt4InARow:
            box(color = color.white, opacity = 1, size = vector(0.775, 0.2, 0.16), pos = vector(0, 0, 0.08) + self.mt4Pos) # Draw the LED box.
        else:
            box(color = color.white, opacity = 1, size = vector(.2, 0.775, 0.16), pos = vector(0, 0, 0.08) + self.mt4Pos) # Draw the LED box.
    ...
    def update(self, mt4Value = "nan"):
        # If we have valid data.
        if mt4Value != "nan":
            # Clip the sensor value if it is out of range.
...
class smallLED():
    def __init__(self, smallLEDPos = vector(0, 0, 0), offColor = color.gray(0.5)):
        ...
        cylinder(color = color.white, opacity = 1, pos = vector(-0.05, 0, 0) + self.smallLEDPos, axis = vector(0, 0, -0.2), radius = 0.01)
...
class bgrLEDBank():
    def __init__(self, bgrLEDBankPos = vector(0, 0, 0), bgrInARow = True, bgThreshold = 5, grThreshold = 30, hysteresis = 0.5):
        self.bgrLEDBankPos = bgrLEDBankPos
        self.bgrInARow = bgrInARow
        self.bgThreshold = bgThreshold
        self.grThreshold = grThreshold
        self.hysteresis = hysteresis # TODO: Adjust thresholds to stabilise the LEDs if a reading is jittering around a boundary.
        if self.bgrInARow:
            self.blueLED  = smallLED(vector(-0.25, 0, 0) + self.bgrLEDBankPos)
            self.greenLED = smallLED(vector(0, 0, 0) + self.bgrLEDBankPos)
            self.redLED   = smallLED(vector(0.25, 0, 0) + self.bgrLEDBankPos)            
        else:
            self.blueLED  = smallLED(vector(0, -0.25, 0) + self.bgrLEDBankPos)
            self.greenLED = smallLED(vector(0, 0, 0) + self.bgrLEDBankPos)
            self.redLED   = smallLED(vector(0, 0.25, 0) + self.bgrLEDBankPos)
    def update(self, sensorValue):
        # If we have valid data.
        if sensorValue != "nan":
            ...
            if self.sensorValue < self.bgThreshold: # Under the blue-green threshold.
                ...
                self.redLED.update()
            if self.bgThreshold <= self.sensorValue <= self.grThreshold: # Within the thresholds for green.
                self.blueLED.update()
                self.greenLED.update(color.green)
                self.redLED.update()
            if self.sensorValue > self.grThreshold: # Over the green-red threshold.
                self.blueLED.update()
                self.greenLED.update()
                self.redLED.update(color.red)

# TODO: Create a tricolour LED class.
...
alertLEDs      = bgrLEDBank(vector(-1.75, 0.75, -0.15), False, 5, 30) # Blue/Green threshold is 5, Green/Red threshold is 30.
